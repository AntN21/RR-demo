// RR-GraphCut.cpp : définit le point d'entrée de l'application.
//

#include "RR-GraphCut.h"
#include "io_png.h"
using namespace std;
// Imagine++ project
...
// Associate index to node (x,y,d).
int index(int x, int y, int d, int nx, int /*ny*/, int nd) {
    return d + nd * (x + nx * y);
}

double rho(double x) {
    return (x > 0) ? sqrt(1 - x) : 1;
}
...
void build_graph(Graph<int, int, int>& G,
    ...
    for (int y = 0; y < ny; y++)
        for (int x = 0; x < nx; x++) {
            int u = win + x * zoom, v = win + y * zoom;
            if (u + dmin + win >= w2 || u + dmax - win < 0)
                continue; // Pixel with no valid patch
            for (int d = 0; d <= nd; d++) {
                // Link to next in line
                int w = INF;
                if (0 <= u + dmin + d - win && u + dmin + d + win < w2) {
                    double ncc = zncc(I1, I1M, I2, I2M, u, v, u + dmin + d, v);
                    w = K + int(wcc * rho(ncc));
                }
                int i = index(x, y, d - 1, nx, ny, nd);
                int j = index(x, y, d, nx, ny, nd);
                if (d == 0)
                    G.add_tweights(j, w, 0);
                else if (d == nd)
                    G.add_tweights(i, 0, w);
                else
                    G.add_edge(i, j, w, 0);
                // Links to neighbors
                if (d == nd) continue;
                if (x + 1 < nx) {
                    i = index(x + 1, y, d, nx, ny, nd);
                    G.add_edge(i, j, lambda, lambda);
                }
                if (y + 1 < ny) {
                    i = index(x, y + 1, d, nx, ny, nd);
                    G.add_edge(i, j, lambda, lambda);
                }
            }
        }
...
int main(int argc, char* argv[]) {
   ...
   /* openWindow(w1 + w2, h);
    ...
    cout << "done" << endl;
    int write_success = io_png_write_f32("output.png", D.getData(), nx, ny, 1);
    
    if (write_success != 0) { printf("Error cant write"); return -1; }
    else { printf("Disparity outputed"); return -1; }
